...
switch ($act) {
    ...
    case 'forget_action':
        if (isset($_POST['submit_email'])) {
            ...
            $usr = new user();
            $checkur = $user->checkEmail($email)->rowCount();
            if ($checkur > 0) {
        ...
        break;
    case 'reset':
        //$_SESSION['email']=array('id': 1651, 'email' banconcop113@gmail.com) if(isset($_POST['submit_password']))
        {
            $pass = $_POST['password']; //1651
            //dò xem pass này có tồn tại trong session ko
            foreach ($_SESSION['email'] as $key => $item) {
                if ($item['id'] == $pass) {
                    $emailold = $item['email']; // lay ra email
